...
from LaMed.src.model.Organ_MAE_SEG import OrganMAEConfig, OrganMAE
...
class MAECLIPTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        seg_loss = outputs.get("seg_loss", None)
        seg_ce_loss = outputs.get("seg_ce_loss", None)
        seg_dice_loss = outputs.get("seg_dice_loss", None)
        ...
        if seg_loss is not None:
            logs["seg_loss"] = float(seg_loss.detach().cpu().mean())
        if seg_ce_loss is not None:
            logs["seg_ce_loss"] = float(seg_ce_loss.detach().cpu().mean())
        if seg_dice_loss is not None:
            logs["seg_dice_loss"] = float(seg_dice_loss.detach().cpu().mean())
    ...
    def training_step(self, model: torch.nn.Module, inputs: dict) -> torch.Tensor:
        """
        Alternate between OrganMAE and Segmentation losses across training steps:
        - Odd steps (1, 3, 5, ...): OrganMAE loss only
        - Even steps (2, 4, 6, ...): Segmentation loss only
        
        This avoids memory corruption from running both decoders simultaneously,
        which causes double encoder calls and DeepSpeed ZeRO state conflicts.
        """
        model.train()
        inputs = self._prepare_inputs(inputs)

        # Save original weights
        orig_mae_w = getattr(model, "mae_loss_weight", 0.0)
        orig_seg_w = getattr(model, "seg_loss_weight", 0.0)

        # If either loss is disabled, use normal behavior
        if orig_mae_w == 0.0 or orig_seg_w == 0.0:
            return super().training_step(model, inputs)

        # Alternate losses based on step number
        # Odd steps: OrganMAE, Even steps: Segmentation
        is_odd_step = (self.state.global_step % 2 == 1)
        
        if is_odd_step:
            # Step 1, 3, 5, ...: OrganMAE only
            model.mae_loss_weight = orig_mae_w
            model.seg_loss_weight = 0.0
        else:
            # Step 2, 4, 6, ...: Segmentation only
            model.mae_loss_weight = 0.0
            model.seg_loss_weight = orig_seg_w

        try:
            # Forward pass (only one decoder active, avoids double encoder call)
            ...
            # Log all losses regardless of step (for monitoring)
            if self.state.global_step % self.args.logging_steps == 0:
                logs = {}
                if outputs.get("mae_loss", None) is not None:
                    logs["mae_loss"] = float(outputs["mae_loss"].detach().cpu().mean())
                if outputs.get("regular_mae_loss", None) is not None:
                    logs["regular_mae_loss"] = float(outputs["regular_mae_loss"].detach().cpu().mean())
                if outputs.get("clip_loss_organ", None) is not None:
                    logs["clip_loss_organ"] = float(outputs["clip_loss_organ"].detach().cpu().mean())
                if outputs.get("clip_loss_caption", None) is not None:
                    logs["clip_loss_caption"] = float(outputs["clip_loss_caption"].detach().cpu().mean())
                if outputs.get("seg_loss", None) is not None:
                    logs["seg_loss"] = float(outputs["seg_loss"].detach().cpu().mean())
                if outputs.get("seg_ce_loss", None) is not None:
                    logs["seg_ce_loss"] = float(outputs["seg_ce_loss"].detach().cpu().mean())
                if outputs.get("seg_dice_loss", None) is not None:
                    logs["seg_dice_loss"] = float(outputs["seg_dice_loss"].detach().cpu().mean())
                if logs:
                    self.log(logs)
...
class ModelArguments:
    ...
    num_clip_organs: int = field(default=0, metadata={"help": "Number of organs to use for organ CLIP loss. Set to 0 or negative to use ALL available organs."})
    ...
    seg_loss_weight: float = field(default=0.0, metadata={"help": "Weight for segmentation loss. Set to 0.0 to disable."})
    seg_mask_dim: int = field(default=1024, metadata={"help": "Dimension of mask projection space for segmentation decoder."})
    enable_CKT: bool = field(default=False, metadata={"help": "Enable Cross-Modal Knowledge Transport: use segmentation embeddings instead of learnable mask_token."})
