...
"""
VitLamMA Volume Processor

Only volume (3D) data processing is supported.
Implementation style follows `architectures/citrus_v_3/volume_processing_citrus_v_3.py`.

Outputs:
- pixel_values_volumes: (num_volumes, S, C, H, W) after preprocessing/patchifying
- volume_grid_thw: (num_volumes, 3) where [T(=S/temporal_patch), H/patch, W/patch] in the vision-token grid
"""
...
# VitLamMA volume IO utilities (paths/nii/dicom/npy loading)
from .vit_lamma_utils import (
    VolumeInput,
    fetch_volume,
    is_valid_volume,
...
def smart_resize_volume(
    ...
    max_pixels: int = 16 * 16 * 8 * 2 * 2 * 2 * 2048, # 8192
) -> Tuple[int, int]:
...
class VitLamMAVolumeProcessorInitKwargs(VideosKwargs):
...
class VitLamMAVolumeProcessor(BaseVideoProcessor):
    """
    Volume-only processor for VitLamMA.

    Input volumes are expected as (S, C, H, W) (or paths that decode to that).
    """
    ...
    valid_kwargs = VitLamMAVolumeProcessorInitKwargs
    model_input_names = ["pixel_values_volumes", "volume_grid_thw"]

    def __init__(self, **kwargs: Unpack[VitLamMAVolumeProcessorInitKwargs]):
    ...
    def _preprocess(
        ...
        **kwargs,
        ...
        for shape, stacked_volumes in grouped_volumes.items():
            ...
            # Derive grid from actual tensor shape so view() always matches numel()
        ...
        # 允许同一 batch 内体积 token 数不同：按 max_N 在 token 维做 padding，避免强制相同 shape。
        # transformers.video_utils.reorder_videos 可能返回：
        # - Tensor (B, N, patch_dim)
        # - list[Tensor]，每个形如 (N, patch_dim) 或 (1, N, patch_dim)
        # 这里统一成 (B, max_N, patch_dim)。
    ...
    def __call__(self, volumes: VolumeInput, **kwargs: Unpack[VitLamMAVolumeProcessorInitKwargs]) -> BatchFeature:
        return self.preprocess(volumes=volumes, **kwargs)

    def preprocess(self, volumes: VolumeInput, **kwargs: Unpack[VitLamMAVolumeProcessorInitKwargs]) -> BatchFeature:
