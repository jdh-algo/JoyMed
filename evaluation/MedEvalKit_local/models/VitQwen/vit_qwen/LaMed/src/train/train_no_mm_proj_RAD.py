...
from LaMed.src.dataset.RAD3D_dataset import VQADataset_train_3D_RAD
...
class DataArguments:
    ...
    # 3D-RAD training data
    use_3D_RAD_data: bool = field(default=False, metadata={"help": "Enable training on 3D-RAD dataset."})
    rad3d_train_data_path: str = field(
        default="/mnt/workspace/offline/shared_data/Medical_Data_3D/3D-RAD/train",
        metadata={"help": "Root directory containing 3D-RAD training CSV files (Task1_*, Task2_*, etc.)"}
    )
    rad3d_train_image_root: str = field(
        default="/mnt/workspace/offline/shared_data/Medical_Data_3D/CT-RATE/dataset/train_fixed",
        metadata={"help": "Root directory for 3D-RAD training images."}
    )
...
def main():
    ...
    if model_args.tune_mm_mlp_adapter:
        ...
        rank0_print(f"[Dataset] Creating training datasets with model_args.img_size={model_args.img_size}")
        
        dataset_list = []
        
        # Add CT-RATE datasets if enabled
        if data_args.use_CT_rate_data:
            rank0_print(f"[Dataset] Adding CT-RATE datasets (CapDataset + VQADataset)")
            base_dataset = UniDatasets(data_args, model_args, tokenizer, mode='train')
            dataset_list.append(base_dataset)
            rank0_print(f"[Dataset] CT-RATE dataset size: {len(base_dataset)}")
        
        # Add 3D-RAD training dataset if enabled
        if data_args.use_3D_RAD_data:
            rank0_print(f"[Dataset] Adding 3D-RAD training dataset from {data_args.rad3d_train_data_path}")
            rad3d_dataset = VQADataset_train_3D_RAD(data_args, model_args, tokenizer, mode='train')
            dataset_list.append(rad3d_dataset)
            rank0_print(f"[Dataset] 3D-RAD dataset size: {len(rad3d_dataset)}")
        
        if not dataset_list:
            raise ValueError("No training data! Set at least one of use_CT_rate_data or use_3D_RAD_data to True.")
        ...
        rank0_print(f"[Dataset] Total training dataset size: {len(train_dataset)}")

    # Create eval dataset - use CT-RATE CapDataset if available, otherwise use a subset of training data
    if data_args.use_CT_rate_data:
        eval_dataset = CapDataset(data_args, model_args, tokenizer, mode='validation')
    elif data_args.use_3D_RAD_data:
        # Use a small subset of 3D-RAD for validation when CT-RATE is disabled
        rank0_print("[Dataset] Using 3D-RAD subset for validation (CT-RATE disabled)")
        from torch.utils.data import Subset
        rad3d_val = VQADataset_train_3D_RAD(data_args, model_args, tokenizer, mode='train')
        # Take first 512 samples for validation
        val_indices = list(range(min(512, len(rad3d_val))))
        eval_dataset = Subset(rad3d_val, val_indices)
    else:
