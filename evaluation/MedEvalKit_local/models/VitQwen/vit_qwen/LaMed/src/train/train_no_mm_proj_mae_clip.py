...
from LaMed.src.model.lamed_arch import _prepare_vision_state_dict, _infer_img_size_from_state, _resize_position_embeddings
...
class ModelArguments:
    ...
    pretrain_vision_model_mae: str = field(default=None, metadata={"help": "Path to pretrained MAE encoder model."})
    pretrain_vision_model_clip: str = field(default=None, metadata={"help": "Path to pretrained CLIP encoder model."})
...
def main():
    ...
    if model_args.vision_tower is not None:
        # Check if we're using dual encoders (MAE + CLIP)
        use_dual_encoders = (model_args.pretrain_vision_model_mae is not None and 
                            model_args.pretrain_vision_model_clip is not None)
        
        if use_dual_encoders:
            rank0_print("="*20 + " Loading Dual Encoders (MAE + CLIP) " + "="*20)
            
            # Initialize config for vision encoders
            model.get_model().config.image_channel = model_args.image_channel
            model.get_model().config.img_size = model_args.img_size
            model.get_model().config.patch_size = model_args.patch_size
            model.get_model().config.vision_tower = model_args.vision_tower
            model.get_model().config.vision_select_layer = model_args.vision_select_layer
            model.get_model().config.vision_select_feature = model_args.vision_select_feature
            
            # Helper function to load a vision encoder
            def load_vision_encoder(checkpoint_path, encoder_name):
                rank0_print(f"Loading {encoder_name} encoder from {checkpoint_path}")
                ...
                if checkpoint_path.endswith(".safetensors"):
                    ...
                    if os.path.isdir(checkpoint_path):
                        # Look for model.safetensors or model.pth in the directory
                        safetensors_path = os.path.join(checkpoint_path, "model.safetensors")
                        pth_path = os.path.join(checkpoint_path, "model.pth")
                        if os.path.exists(safetensors_path):
                            raw_state = load_file(safetensors_path, device="cpu")
                ...
                # Build vision tower
                vision_tower = build_vision_tower(model.get_model().config)
                vision_tower.requires_grad_(not model_args.freeze_vision_tower)
                
                # Handle position embeddings resize if needed
                pos_key = "patch_embedding.position_embeddings"
                target_grid = tuple(max(1, model_args.img_size[i] // model_args.patch_size[i]) for i in range(3))
                if pos_key in vision_ckpt:
                    inferred_img, ckpt_grid = _infer_img_size_from_state(
                        vision_ckpt,
                        tuple(model_args.patch_size),
                        tuple(model_args.img_size),
                    )
                    resized = _resize_position_embeddings(
                        vision_ckpt[pos_key],
                        ckpt_grid,
                        target_grid,
                        vision_tower.vision_tower.patch_embedding.position_embeddings,
                    )
                    vision_ckpt[pos_key] = resized
                
                # Load state dict
                missing, unexpected = vision_tower.vision_tower.load_state_dict(vision_ckpt, strict=False)
                rank0_print(f"Loaded {encoder_name} encoder from {checkpoint_path}")
                if missing:
                    rank0_print(f"[{encoder_name}] Missing keys: {missing}")
                if unexpected:
                    rank0_print(f"[{encoder_name}] Unexpected keys: {unexpected}")
                
                return vision_tower
            
            # Load both encoders
            mae_encoder = load_vision_encoder(model_args.pretrain_vision_model_mae, "MAE")
            clip_encoder = load_vision_encoder(model_args.pretrain_vision_model_clip, "CLIP")
            
            # Create a dual encoder wrapper that concatenates outputs
            class DualEncoderWrapper(nn.Module):
                ...
                def forward(self, images):
                    ...
                    # ViT3DTower.forward returns image_features directly (not a tuple)
                    mae_features = self.mae_encoder(images)
                    clip_features = self.clip_encoder(images)
                    
                    # Concatenate along feature dimension (dim=-1)
                    # Both should have same sequence length (num_patches), concatenate features
                    combined_features = torch.cat([mae_features, clip_features], dim=-1)
            ...
            # Create dual encoder wrapper
            dual_encoder = DualEncoderWrapper(mae_encoder, clip_encoder)
            model.get_model().vision_tower = dual_encoder
            
            vision_hidden_size = dual_encoder.hidden_size
            rank0_print(f"MAE encoder hidden size: {mae_encoder.hidden_size}")
            rank0_print(f"CLIP encoder hidden size: {clip_encoder.hidden_size}")
            rank0_print(f"Combined hidden size: {vision_hidden_size}")
        ...
        # Replace mm_projector with direct connection to connect vision encoder directly to LLM
        llm_hidden_size = model.config.hidden_size
        
        # Calculate number of vision tokens (patches) from image_size and patch_size
        ...
        # Replace the mm_projector with direct connection
        direct_conn = DirectConnection(vision_hidden_size, llm_hidden_size, num_patches)
        model.get_model().mm_projector = direct_conn
        
        rank0_print(f"Replaced mm_projector with direct connection for LoRA fine-tuning")
        rank0_print(f"Vision hidden size: {vision_hidden_size}, LLM hidden size: {llm_hidden_size}")
        rank0_print(f"Vision tokens (proj_out_num): {direct_conn.proj_out_num}")
