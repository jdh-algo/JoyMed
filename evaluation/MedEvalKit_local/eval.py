...
def main():
    ...
    os.environ["OPENAI_API_BASE"] = api_map[args.judge_gpt_model]["API_URL"]
    os.environ["OPENAI_API_KEY"] = api_map[args.judge_gpt_model]["TOKEN"]
    ...
    args.output_path = os.path.join(args.output_path, os.path.basename(args.prompt_version).replace('.json','')) \
                       if os.path.isfile(args.prompt_version) else os.path.join(args.output_path, args.prompt_version)
    ...
    for eval_dataset in args.eval_datasets:
        matric_path = os.path.join(args.output_path,eval_dataset,"metrics.json")
        if not(os.path.exists(matric_path) and None not in json.load(open(matric_path,'r')).values()):
            task_finished = False
    ...
    if task_finished:       
        for eval_dataset in args.eval_datasets:
            ...
            total_results[eval_dataset] = metrics
        ...
        for eval_dataset in tqdm(args.eval_datasets):
            set_seed(args.seed)
            # print("随机数测试:")
            # print("torch随机数:", torch.rand(3))
            # print("numpy随机数:", np.random.rand(3))
            # print("random随机数:", [random.random() for _ in range(3)])
            task_id = os.path.basename(os.path.dirname(args.output_path)).split('_')[0]
            print(f'evaluating on {task_id}-{eval_dataset}-{os.environ["chunk_idx"]}...')
