...
def _evaluate_core(out_samples, desc, category_key, category_name_mapping=None, calculate_overall=True):
    ...
    for pred_item in tqdm(out_samples, desc=desc):
        try:
            if "conversations" in pred_item:
                gt_value = pred_item['conversations'][1]['value']
                question = pred_item['conversations'][0]['value']
            else:
                gt_value = pred_item['answer_idx'] if "answer_idx" in pred_item else pred_item['answer'] if "answer" in pred_item else pred_item['report']
        ...
        if openclose_type in ['OPEN', 'REPORT']:
        ...
        elif openclose_type in ['CHOICE']:
            if category is not None:
                closed_questions_count_by_type[category] += 1

            if "choices" in pred_item:
                is_correct = judge_multi_choice(pred_item["choices"],gt_value,pred_value)
            elif "options" in pred_item:
                choices = pred_item["options"]
                choices = [f"{key}.{value}" for key,value in choices.items()]
                pred_item["choices"] = choices
                is_correct = judge_multi_choice(pred_item["choices"],gt_value,pred_value)
                
            elif "conversations" in pred_item:
                pred_item["choices"] = question.split('\n')[1:-1]
                is_correct = judge_multi_choice(pred_item["choices"],gt_value,pred_value)
    ...
    for cat_key in all_categories:
        ...
        category_name = category_name_mapping.get(cat_key, cat_key) if category_name_mapping is not None else cat_key
        ...
        if open_count==0 and closed_count>0:
            metrics_result[category_name] = {"accuracy": closed_acc * 100, "total": closed_count, "correct": closed_correct}
        elif open_count>0 and closed_count==0:
            metrics_result[category_name] = {k: v * 100 for k, v in open_metrics.items()}
            metrics_result[category_name]["total"] = open_count
        else:
            metrics_result[category_name] = {
