...
import numpy as np
import sys
from utils.eval_metrics.seg_report import generate_seg_file
from utils.eval_metrics.eval_RadSEM import eval_report
...
class Evalreport:
    def __init__(self):
        ...
        self.dataset_path = '-'.join([tmp for tmp in self.dataset_path.split('-') if (tmp != "MM" and tmp != "Report" and tmp != "VQA")])
        self.output_path = os.environ.get("output_path",None)
        
    def eval_rouge(self, datas):
    ...
    def __call__(self, datas):
        if datas[0].get('report_eval_method','') == "RadSEM":
            #数据库gt的report句子进行分割，并把结果保存在report_seg_path中，如果已经分割会跳过
            gt_seg_path = os.path.join(self.dataset_path, 'gt_report_seg.jsonl') #提前准备好
            generate_seg_file(datas, gt_seg_path)
            #将分割后的gt和模型输出response作为输入，使用RadSEM方法评估
            pred_seg_path = os.path.join(self.output_path, 'pred_report_seg.jsonl')
            eval_tag_path = os.path.join(self.output_path, 'eval_tag.jsonl')
            output_score_path = os.path.join(self.output_path, 'eval_score.jsonl')
            metrics = eval_report(gt_seg_path, datas, pred_seg_path, eval_tag_path, output_score_path)
        else:
            metrics = self.eval_rouge(datas)
