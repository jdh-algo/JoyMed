...
class MedXpertQA(BaseDataset):
    def __init__(self,model,dataset_path,output_path,split = "MM"):
        ...
        if "rag_bench" in self.prompt_version:
            self.dataset_path = os.path.join("/mnt/workspace/offline/sunqintian3/mm_pipeline/data_juicer/input/RAG", self.prompt_version, "MedXpertQA")
        else:
    ...
    def load_data(self):
        ...
        if "rag_bench" in self.prompt_version:
            dataset = pd.read_json(os.path.join(dataset_path,"MM/test.jsonl"), lines=True).to_dict(orient="records")
            # dataset = load_dataset("json", data_files=os.path.join(dataset_path,"MM/test.jsonl"),split="train")
        else:
    ...
    def construct_messages(self,sample):
        ...
        if "Answer Choices:" in question:
            question = question[:question.index("Answer Choices:")].strip()
