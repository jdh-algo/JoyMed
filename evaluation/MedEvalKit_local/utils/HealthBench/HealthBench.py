...
class HealthBench(BaseDataset):
    ...
    def cal_metrics(self,out_samples):
            ...
            for rubric_item in tqdm(rubric_items):
                ...
                while True:
                    ...
                    if not grading_response:
                        # continue
                        print("grading_response is None, return None")
                        sample["metrics"] = None
        ...
        results_deal_path = os.path.join(self.output_path,"results_deal.json")
        if os.path.exists(results_deal_path):
            try:
                out_samples = json.load(open(results_deal_path,'r'))
            except:
                tasks = []
                for sample in out_samples:
                    tasks.append(deal_sample(sample))
                out_samples = asyncio.run(deal_tasks(tasks))
                save_json(results_deal_path,out_samples)
